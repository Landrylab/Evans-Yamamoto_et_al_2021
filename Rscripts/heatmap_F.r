...
heatmap = ggplot(data )+   
    geom_tile(aes(prey_ID,bait_ID,fill = log10(F)))  +
    theme_bw()   + 
...
th = theme(panel.spacing = unit(1, "lines"),
    panel.border = element_rect(fill=NA, colour = "black", size=1),
...
histogram  = ggplot(data) +   
    geom_histogram(aes(x=log10(F),y=log10(..count..+1)),bins=30,fill="#2f4893",color="#999999",size=0.1) + 
    theme_bw()  +
    xlab(bquote(~Log["10"]~ '('  ~italic("F")["Fused barcodes"]~ ')'))+
    ylab(bquote(~Log["10"]~ '(Strain counts + 1)'))+
    facet_grid(Replicate~Condition)

histogram2 = histogram+th+  
    ...
    scale_x_continuous(expand=c(0,0),limits=c(round(log10(min(data$F))-1,digit=0)-0.1,round(log10(max(data$F))+1,digits=0)+0.1),breaks=seq(round(log10(min(data$F))-2,digit=0),round(log10(max(data$F))+2,digits=0)+0.05,1))  +th
    


hs   = gsub("heatmap", "histogram", out)

pl = histogram2+th+theme(aspect.ratio=0.35)
